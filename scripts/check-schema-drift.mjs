...
//
// Migration sources (both are read and merged):
//   prisma/migrations/*/migration.sql   — Prisma auto-migrations (subdirectory format)
//   prisma/sql/*.sql                    — manual named SQL files (001_*, 002_*, etc.)
//
// What it detects:
...
//   • Auto-generated UNIQUE indexes ending in `_key` are suppressed
...
//   node scripts/check-schema-drift.mjs --table=students  # filter one table
...
function loadMigrations() {
  ...
  // Prisma auto-migrations: prisma/migrations/<timestamp_name>/migration.sql
  const prismaDir = resolve(ROOT_DIR, "prisma/migrations")
  if (existsSync(prismaDir)) {
    const subdirs = readdirSync(prismaDir, { withFileTypes: true })
      .filter(d => d.isDirectory() && !d.name.startsWith("."))
      .sort((a, b) => a.name.localeCompare(b.name))
    for (const subdir of subdirs) {
      const sqlPath = join(prismaDir, subdir.name, "migration.sql")
      if (existsSync(sqlPath)) {
        files.push({ name: `migrations/${subdir.name}/migration.sql`, path: sqlPath })
      }
    }
  }

  // Manual SQL files: prisma/sql/*.sql (001_foundation, 002_ecommerce, etc.)
  const sqlDir = resolve(ROOT_DIR, "prisma/sql")
  if (existsSync(sqlDir)) {
    const sqlFiles = readdirSync(sqlDir)
      .filter(f => f.endsWith(".sql"))
      .sort()
    for (const f of sqlFiles) {
      files.push({ name: `sql/${f}`, path: join(sqlDir, f) })
    }
  }

  if (files.length === 0) {
    console.warn("No migration files found. Expected prisma/migrations/ or prisma/sql/")
  }
...
;(async () => {
  ...
  if (totalDrift === 0) {
    ...
    md.push("")
    md.push(`Every suggested fix concatenated. Paste into prisma/sql/ to bring migrations back in sync:`)
    md.push("")
    md.push("```sql")
