 ...
 *
 * Ported with the statusline it probes, from the sibling project that built both. The
 * library-import collision below is a DEMONSTRATED failure there, not a hypothetical one here.
 *
...
// ── THE GITIGNORE TRAP ───────────────────────────────────────────────────────────────────────
// Named as a hard constraint by dev-standards/playbooks/3-TOKEN-ECONOMY.md §2, and it has bitten
// the sibling project: a `.claude/*` ignore with per-directory negations swallowed files that were
// then "committed" only in the commit message. This repo does not use that style TODAY — it ignores
// two specific paths — so the trap is currently disarmed by accident rather than by decision. That
// is exactly the state worth pinning: the day someone tightens .gitignore to `.claude/*`, this fires
// instead of the statusline silently ceasing to exist for every other machine.
//
// Asserted against the IGNORE RULES, not against tracked-ness. A tracked-ness assertion cannot pass
// on the commit that introduces the file, so it would have to be added a commit later — which is to
// say, never.
{
  const ignored = (p) => spawnSync("git", ["check-ignore", "-q", p], { encoding: "utf8" }).status === 0

  ok(!ignored(LINE),
    "the statusline is NOT gitignored — an ignored one exists on this machine and nowhere else",
    "a .claude/* ignore needs an explicit `!.claude/statusline.js` negation")

  ok(!ignored(".claude/hooks/context-budget.js"),
    "…nor is the hook it reads its cumulative figures from")

  // The other direction, and it fails the opposite way: this one MUST be ignored. It holds byte
  // offsets into one machine's transcript path, so a committed copy is not stale, it is wrong —
  // and the hook rewrites it on every prompt.
  ok(ignored(".claude/.context-budget.state.json"),
    "the hook's state file IS gitignored — per-session byte offsets must never travel between machines",
    "add `.claude/.context-budget.state.json` to .gitignore")
}
