...
/* LT's reading, 2026-09-09, CLAUDE.md at b4617a0. Both headings exist verbatim —
 * `### Enforced` at line 305, `## 5 · DOCTRINE THE MACHINE CANNOT HOLD` at line 330 —
 * so this is canon's defaults unedited. Recorded rather than left implicit, because
 * "I didn't change it" and "I checked and it matched" look identical in a diff and
 * only one of them is a reading. The middle dot in §5's heading is U+00B7, not a
 * hyphen; exact full-line equality means a look-alike here reports the section as
 * vanished, which is the loud failure and the right one. */
 ...
 */
/* LT writes TWO digits — `### M-01` … `### M-06`, six entries at docs/MECHANISABLE.md.
 * Canon's default heading and pointer grammars already admit that (`\d{2,3}`), so both
 * are taken unedited and only the samples change. They must be in THIS project's
 * grammar, and three digits would be a different one — the fixtures would still pass,
 * which is exactly why the samples are asserted against the grammar rather than
 * trusted. M-99 is absent from the real register on purpose and must stay absent. */
const SAMPLE_IDS = ["M-01", "M-02", "M-99"]
/* KIT:CONFIG /register */
...
// "" — LT ships NO custom ESLint rules. There is no `eslint-rules/` directory and no plugin
// registered in eslint.config.mjs; the one `eslint:` marker here claims a stock rule,
// `@typescript-eslint/no-explicit-any`. ESLINT_RULES_DIR keeps canon's default because with an
// empty prefix nothing reaches it — a value that is never read, left at canon's so a diff against
// canon stays about the values that ARE read.
const ESLINT_CUSTOM_PREFIX = ""
const ESLINT_RULES_DIR = "eslint-rules"
const SCRIPTS_DIR = "scripts"
// One gate here, not two. `npm run check` is the whole gauntlet (§3) and there is no `check:full`.
const GATE_SCRIPTS = ["check"]
const HOOKS_DIR = ".claude/hooks"
const SETTINGS_PATH = ".claude/settings.json"
const CI_DIR = ".github/workflows"
const AUDIT_ENTRY = "scripts/architecture-audit.mjs"

/* ── TWO NAMESPACES, AND ONE OF THEM OVERRIDES CANON (M-KIT-12, M-KIT-13) ──────────────
 *
 * `audit:` IS AN OVERRIDE, NOT AN ADDITION, and without it this project's gate is red with
 * twenty false findings. Canon's arm is `rd(AUDIT_ENTRY).includes(id)` — a literal substring
 * test — and LT's audit ids are SLUGGED check names: `check("date-safety: no hardcoded +02:00
 * offset")` is tagged `audit:date-safety-no-hardcoded-02-00-offset`. Measured against the 21
 * `@enforced audit:` markers in CLAUDE.md, canon's arm resolved 1, and the 1 was a FALSE
 * POSITIVE — it matched a selftest fixture literal at architecture-audit.mjs:152, not a check.
 * So the literal test was 0-for-21 on the thing it was asked about and looked 1-for-21.
 *
 * The resolver below derives the same slugs the audit derives, from the same file, by the same
 * transformation — `slug()` at architecture-audit.mjs:115, copied here as four lines rather than
 * imported, because importing the audit runs it. If that transformation changes, this stops
 * resolving and says so; that is the failure mode to want.
 *
 * It resolves through the thing that INVOKES the control, which is canon's shape: a `check(...)`
 ...
 *
 * Both `mustResolve` values are live claims from this repo, so a resolver that stops reading its
 * subject fails on the day it stops rather than the day someone notices. */

const auditCheckSlugs = (io) => {
  const slug = (s) => s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")
  return new Set([...io.read(AUDIT_ENTRY).matchAll(/\bcheck\(\s*"([^"]+)"/g)].map((m) => slug(m[1])))
}

PROJECT_RESOLVERS.audit = {
  resolve: (id, io) => auditCheckSlugs(io).has(id),
  mustResolve: "date-safety-no-hardcoded-02-00-offset",
  mustNotResolve: "no-check-is-named-this",
}

// The tag -> the exact `permissions.ask` pattern it claims. A tag naming no pattern resolves
// false, which is right: an unmapped twin is an unproven twin.
const SETTINGS_TAGS = { "ask-git-push": "Bash(git push*)" }

PROJECT_RESOLVERS.settings = {
  resolve: (id, io) => {
    const claim = SETTINGS_TAGS[id]
    return claim ? (io.json(SETTINGS_PATH).permissions?.ask ?? []).includes(claim) : false
  },
  mustResolve: "ask-git-push",
  mustNotResolve: "nothing-asks-for-this",
}
