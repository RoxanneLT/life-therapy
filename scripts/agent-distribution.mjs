...
if (process.argv.includes("--selftest")) {
  ...
  ok(slugFor("c:\\dev\\life-therapy") === "c--dev-life-therapy", "slugFor matches Claude Code's project-directory naming", slugFor("c:\\dev\\life-therapy"))
...
if (generation === null) {
  ...
  // This project has no docs/EXPERIMENTS.md: its E1/E1b/E2/E3 records live in the CLAUDE.md header
  // comment. A distribution table does not fit there, so the honest instruction is to start the
  // file rather than to point at one that does not exist — a citation nobody can follow is worse
  // than an instruction to create the target.
  console.log(`   this table in docs/EXPERIMENTS.md as E4 (start the file — this repo's E1-E3 live in`)
  console.log(`   the CLAUDE.md header comment), then tighten budgets`)
