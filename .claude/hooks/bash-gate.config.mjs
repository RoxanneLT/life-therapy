 ...
 *
 * ── THIS PROJECT'S TWO VALUES, migrated 2026-09-09 from the two regions this file replaces ────
 *
 * They were already EQUAL at `master` in both places, so life-therapy was never in the silent
 * state described above — this migration removes the way to enter it, it does not repair a hit.
 * Recorded because "was never broken" and "was not checked" look identical afterwards, and the
 * whole point of the file is that only one of them stays true by construction.
 *
 * WORKING is deliberately NOT a branch this repo currently has. Branches come and go, and a probe
 * input naming a live one starts failing for a reason that has nothing to do with the gate.
 * `feature/x` is the spelling canon's own per-segment case already uses. */
export const PROTECTED_BRANCH = "master";
export const WORKING_BRANCH = "feature/x";
