 ...
 */
/* LT's reading, 2026-09-09, `.claude/agents/` clean at b0873a1 (last touched b0d525b).
 * Six spines, and they ARE the six canonical ones, so this table is canon's defaults
 * unedited — recorded rather than left implicit, because "I didn't change it" and "I
 * checked and it matched" look identical in a diff and only one of them is a reading.
 *
 *   grounder · census · walker · db-inspector   `tools:` carries Write; each spine directs
 *                                               its one artefact to `.handoff/<task-slug>/`
 *   crawler-doctrine   `tools: Read, Grep, Glob` — and the harness advertises it as
 *                      `Read, Grep, Glob, Write, Edit` (E8 read off the live agent list, not
 *                      inferred). Its spine: "Emits JSON findings only, never prose, never
 *                      edits." So `[]`, and this is the L-62 over-grant LT actually HAD:
 *                      §7's table said `.handoff` for it, while its own comment argued at
 *                      length that it never writes anything. The prose list was the wrong
 *                      authority here too.
 *   implementer        `tools: … Edit, Write …`; editing source is the remit
 */
const SCOPES = {
  grounder: [".handoff"],
  census: [".handoff"],
