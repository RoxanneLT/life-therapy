...
| L-68 | 2026-09-11 | `CLAUDE.md` §7, *"STANDING AUTHORISATION — Stéan, 2026-09-11, from this date onwards"*, in Stéan's own words: *"from today onwards i authorise you to use/spawn agents independently"*. Queued 2026-09-10 by the triage session, which arrived with *"Do not use the Agent tool, workflows or deep-research unless requested"* and spawned nothing. The session waited for the owner's words and did not draft them. The words name agents, so workflows and deep-research still wait to be asked; the section says so. It lifts no limit: the write scope, the implementer's manifest, the denial of commits from a subagent, `census` as the only spawner, depth 2 and the push gate all stand. It takes canon's rules of discretion from `4-AGENT-PIPELINES.md` §3.1, as yoros did. It does not repeat the search L-68 records |
...
| `bash-gate` v4 · `check-hook-registration` v4 · `check-handoff-contract` v4 · `check-install-platform` v2 · `check-claude-md` v16 · `check-brief` v6 · `dates` v2 · `dates-test` v3 | as listed | moved 2026-09-10 by each pin's own route, from canon's history at `efbf834` (M-KIT-28): canon's bytes with this project's `KIT:CONFIG` regions put back by slug. **All eight 2026-09-24 pins are now finished work: drop them from `ledgers/projects.json`** | the commit that adds this row. Before the move, each copy equalled canon's `e006249^` bytes plus LT's regions exactly, so there was no local fork to lose. After it, each file's diff equals canon's own `e006249^..HEAD` diff for that file (8 of 8). `npm run check` → exit 0: `bash-gate: 65 probes pass`, the scar suite 17 of 17, 231 tests with 0 failing. The one local edit is inside `bash-gate`'s `twins` region: the two dated `@probed-kit` records move to v4 with their dates kept, because v4 changed `SHORT_CLUSTER_WITH_F`, which `isForcePush` reads. It was measured equivalent over 21,435,888 strings. `check-kit-drift` → all eight `STALE IN THE AHEAD DIRECTION` |
